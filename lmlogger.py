        ...
        with self.process_lock:
            if self.llm_process is None or self.llm_process.poll() is not None:
                try:
                    ...
                    print(f"Error starting LLM process: {e}", file=sys.stderr)
                    self.llm_process = None
    ...
    def generate(self, prompt):
        ...
        with self.process_lock:
            ...
            try:
                ...
                print(f"Error writing to process: {e}", file=sys.stderr)
                self.llm_process = None
                self.generate(prompt)  # Рекурсивный вызов после перезапуска
